...
//			// Price is replaced with Salary
//			Price:    &item.Salary,
//			Category: jsonData.Domain,
//			// Source and Timestamp are replaced with Company and Location
//			Source:    &item.Company,
//			Timestamp: &item.Location,
//		})
//	}
//
//	return data, nil
...
//
//func main() {
//	jsonFile := "C:\\Users\\Public\\GoLandProjects\\PredictAi\\crab\\indeed_jobs.json"
//
//	// Load data from JSON
//	jobData, err := LoadDataFromJSON(jsonFile)
//	if err != nil {
//		fmt.Println("Error loading data from JSON:", err)
//		os.Exit(1)
//	}
...
//	rand.Shuffle(len(jobData), func(i, j int) {
//		jobData[i], jobData[j] = jobData[j], jobData[i]
//	})
//
//	// Split data into training and testing sets (80/20 split)
//	trainDataSize := int(float64(len(jobData)) * 0.8)
//	trainData := jobData[:trainDataSize]
//	testData := jobData[trainDataSize:]
//
//	// Print the sizes of trainData and testData
//	fmt.Printf("Training data size: %d\nTesting data size: %d\n\n", len(trainData), len(testData))
//
//	// Print a few entries from testData
//	fmt.Println("Sample test data entries:")
//	for i := 0; i < 3 && i < len(testData); i++ {
//		fmt.Printf("Title: %s\nDescription: %s\n\n", *testData[i].Title, *testData[i].Description)
//	}
//
...
//
//	// Evaluate classifier with testing data
//	for _, item := range testData {
//		if item.Description != nil && item.Title != nil {
//			text := *item.Title + " " + *item.Description // Combine title and description
//			skills := preprocessText(text)
//
//			predictedIndustry, probabilities := classifier.PredictWithProbabilities(skills)
//			fmt.Printf("Title: %s\nSkills: %v\nPredicted Industry: %s\nProbabilities: %v\n\n", *item.Title, skills, predictedIndustry, probabilities)
//		}
//	}
//}

//func main() {
//	jsonFile := "C:\\Users\\Public\\GoLandProjects\\PredictAi\\crab\\indeed_jobs.json" // Update this path accordingly
//
//	// Load data from JSON
//	jobData, err := LoadDataFromJSON(jsonFile)
//	if err != nil {
...
//	classifier := NewNaiveBayesClassifier()
//	classifier.Train(jobData)
//
//	// Test cases
//	testCases := [][]string{
//		{"network security", "malware analysis", "incident response"},
//		{"data privacy", "legal compliance", "risk assessment"},
//		{"cloud security", "vulnerability management", "encryption"},
//		{"penetration testing", "ethical hacking", "cybersecurity"},
//		{"security operations", "threat intelligence", "SIEM"},
//	}
//
//	// Evaluate test cases
//	for i, skills := range testCases {
//		predictedIndustry, _ := classifier.PredictWithProbabilities(skills)
//		fmt.Printf("Test Case %d - Skills: %v\nPredicted Industry: %s\n\n", i+1, skills, predictedIndustry)
//		fmt.Println()
//	}
//}
...
//			fmt.Println("Description:", *item.Description)
//		}
//		if item.Salary != nil && *item.Salary != "" { // Replaced Price with Salary
//			fmt.Println("Salary:", *item.Salary)
//		}
//		if item.Company != nil && *item.Company != "" { // Added Company
//			fmt.Println("Company:", *item.Company)
//		}
//		if item.Location != nil && *item.Location != "" { // Added Location
