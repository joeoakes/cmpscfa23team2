...
func TestHashPassword(t *testing.T) {
	...
	if err != nil {
		...
		dal.InsertLog("400", "Failed to hash password", "TestHashPassword()")
	} else {
		dal.InsertLog("200", "Successfully hashed password", "TestHashPassword()")
	}

	err = bcrypt.CompareHashAndPassword(hashed, []byte(password))
	if err != nil {
		t.Errorf("Hashed password does not match the original password: %v", err)
		dal.InsertLog("400", "Failed to compare hashed password", "TestHashPassword()")
	} else {
		dal.InsertLog("200", "Successfully compared hashed password", "TestHashPassword()")
	}
}
...
func TestComparePassword(t *testing.T) {
	...
	if err != nil {
		...
		dal.InsertLog("400", "Failed to compare password", "TestComparePassword()")
	} else {
		dal.InsertLog("200", "Successfully compared password", "TestComparePassword()")
	}
...
func TestGenerateToken(t *testing.T) {
	...
	if err != nil {
		...
		dal.InsertLog("400", "Failed to generate token", "TestGenerateToken()")
	} else {
		dal.InsertLog("200", "Successfully generated token", "TestGenerateToken()")
	}

	valid, err := dal.ValidateToken(token)
	if err != nil {
		t.Errorf("Token validation failed: %v", err)
		dal.InsertLog("400", "Failed to validate token", "TestGenerateToken()")
	} else {
		dal.InsertLog("200", "Successfully validated token", "TestGenerateToken()")
	}

	if !valid {
		t.Errorf("Token is not valid.")
		dal.InsertLog("400", "Token is not valid", "TestGenerateToken()")
	}
}
...
func TestAuthenticateUser(t *testing.T) {
	...
	if err != nil {
		...
		dal.InsertLog("400", "Failed to hash password", "TestAuthenticateUser()")
	}
	...
	if err != nil {
		...
		dal.InsertLog("400", "Failed to create user", "TestAuthenticateUser()")
	}
	...
	if authErr != nil {
		...
		dal.InsertLog("400", "Authentication failed", "TestAuthenticateUser()")
	} else {
		dal.InsertLog("200", "Authentication succeeded", "TestAuthenticateUser()")
	}
...
//	oldRefreshToken := "$2a$10$BuKCVGURqnLBoVKo9IgDNuwh7JW8G406xdGmYqATmQAp5qIY4eXQa"
//
...
//	if err != nil {
//		log.Printf("Using refresh token: %s", oldRefreshToken)
